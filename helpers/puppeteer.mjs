...
/**
 * @module helpers/puppeteer
 *
 * UA/Client-Hints + Navigator-surface tooling for Puppeteer, plus a small set
 * of quality-of-life patches for injecting local scripts under strict CSP.
 *
 * ## What this module gives you
 * - {@link applyUAOnNetwork}: CDP-level UA + UA-CH (Client Hints) headers
 * - {@link installNavigatorShims}: JS-surface shims (navigator, UA-CH, etc.)
 * - {@link applyProfileToPage}: convenience to apply both + set viewport
 * - {@link installPageExtensions}: adds `page.injectScript()` and `page.injectJquery()`
 *
 * All helpers are **idempotent** and safe to call more than once.
 *
 * @example <caption>Apply a realistic 2025 UA+CH profile to a new page</caption>
 * import puppeteer from "puppeteer";
 * import { pick2025UA } from "./ua.mjs";
 * import { applyProfileToPage } from "./helpers/puppeteer.mjs";
 *
 ...
 * await browser.close();
 *
 * @example <caption>Install page extensions and inject a local helper despite CSP</caption>
 * import puppeteer from "puppeteer";
 * import { installPageExtensions } from "./helpers/puppeteer.mjs";
 *
 * const browser = await puppeteer.launch();
 * const page = await browser.newPage();
 * installPageExtensions(page);     // patches Page.prototype once per process
 *
 * await page.goto("https://example.org", { waitUntil: "domcontentloaded" });
 * await page.injectScript("lib/jquery.min.js");   // read from disk → inline script
 * const ver = await page.evaluate(() => window.jQuery?.fn?.jquery);
 ...
 * @param {import('../helpers/ua.mjs').UAProfile} profile
 *
 * @example
 * const profile = pick2025UA();
...
export async function applyUAOnNetwork(page, profile) {
  ...
  const brand =
    /Edge/i.test(profile.browserName) ? "Microsoft Edge" :
...
export async function installNavigatorShims(page, profile) {
  ...
  await page.evaluateOnNewDocument(
    (params) => {
      const {
        uaString, os, isMobile, arch, clientHints, viewport,
        hwCores, memGB, model
      } = params;
      ...
      // We simulate what our network headers claim.
      const major = (() => {
        const m = uaString.match(/(?:Chrome|Chromium|Edg)\/(\d+)/);
        return m ? m[1] : "120";
      })();

      const realBrand = /Edg\//.test(uaString)
        ? "Microsoft Edge"
        : /Chromium\//.test(uaString)
        ? "Chromium"
        : "Google Chrome";
      ...
      const uaData = {
        ...
        toJSON() {
          return { brands, mobile: isMobile, platform: os };
        },
      };

      define(navigator, "userAgentData", uaData);

      // Stealthy tweaks (headless hints)
      try {
      ...
      // Permissions stealth (notifications)
 ...
 *
 * @example <caption>Patch once and use on any page</caption>
 * import puppeteer from "puppeteer";
 * import { installPageExtensions } from "./helpers/puppeteer.mjs";
 *
 * const browser = await puppeteer.launch();
 * const page = await browser.newPage();
 * installPageExtensions(page); // patches Page.prototype only once
 *
 * await page.goto("https://example.org", { waitUntil: "domcontentloaded" });
 * await page.injectScript("lib/jquery.min.js");
 * const text = await page.evaluate(() => window.jQuery("h1").text());
 * console.log(text);
 */
export function installPageExtensions(page) {
   ...
   * await page.injectScript("lib/jquery.min.js");
   *
   * @example <caption>Using a readiness predicate</caption>
   * await page.injectScript("lib/jquery.min.js", {
   *   readyFunction: () => !!window.jQuery && !!window.jQuery.fn?.jquery
   * });
   */
   ...
   *
   * @example
   * await page.injectJquery({ jqueryPath: "lib/jquery.min.js" });
   * const text = await page.evaluate(() => window.$jq("h1").text());
   */
  if (!proto.injectJquery) {
    proto.injectJquery = async function injectJquery({
      ...
      if (!resolvedPath) {
        ...
        // Node 20+/22+: import.meta.resolve is available.
...
export default {
  applyUAOnNetwork,
  installNavigatorShims,
  applyProfileToPage,
  installPageExtensions,
  clickPossiblyNavigates,
  safeEvaluate,
  startMultiTap,
  startTapAll,
  clickIfVisible,
